...
Inputs from dispatch: Use the task contract, dispatch material, accepted lineage artifacts, attempt history, the accepted Checker baseline, and the Fixer evidence. Preserve the original finding IDs and baseline digest.

Readable assets: Read the selected entrypoint, Doublechecker core skill, context router output, required dispatch material, original criteria, and every declared review source before judging. Treat checkout evidence as immutable evidence and retain its provenance.
