---
name: execution-consultant-core
description: Use when executing the Consultant stage for a selected Millrace execution workflow.
---
...
| `summary` | yes | string | Consultant decision summary. |
...
| `summary` | yes | string | Decision evidence summary. |

`execution.artifacts.incident_report`

| Field | Required | Type | Meaning |
| --- | --- | --- | --- |
| `artifact_kind` | yes | string | Must be `execution.artifacts.incident_report`. |
| `summary` | yes | string | planning-facing incident summary. |
...
artifact_id:
artifact_kind:
produced_by_stage: lad_consultant
source_work_item_id:
source_run_id:
terminal_marker:
summary:
...
```text
artifact_id: lad_consultant-result-1
artifact_kind: execution.artifacts.stage_result
produced_by_stage: lad_consultant
source_work_item_id: work-1
source_run_id: run-1
terminal_marker: CONSULT_COMPLETE
summary: Preserved evidence and made a bounded local-continuation decision with assumptions listed.
fields:
  artifact_kind: execution.artifacts.stage_result
  summary: Preserved evidence and made a bounded local-continuation decision with assumptions listed.
evidence:
...
- Escalates with a vague incident that lacks source task and failure evidence.
...
- Terminal marker is legal for `lad_consultant` in the selected workflow.
