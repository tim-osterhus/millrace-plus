...
- Set `AwardDecision.source_request_id` to the exact selected `CandidateBundle.source_requirement_id` for every award decision.
...
- `AwardDecision.source_request_id` must exactly equal `CandidateBundle.source_requirement_id`. Do not look up or reconstruct this identity from lineage, runtime evidence, storage, or any other artifact.
