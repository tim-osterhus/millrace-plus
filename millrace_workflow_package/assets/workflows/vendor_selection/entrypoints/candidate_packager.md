...
Scope:
- You own: Package the selected candidate bundle for parallel rubric and conflict checks.
...
Readable assets:
- Open `vendor_selection.skills.candidate_packager_core` for exact artifact schemas, selected marker protocol, examples, and validation checklist.
- For catalog facts, use only selected package data shown in the core skill.

Writable artifacts:
- Return the exact selected artifact JSON object for the chosen marker. Selected schemas for this stage: CandidateBundle.
