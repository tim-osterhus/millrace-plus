...
Scope:
Check whether the selected request is policy-allowed using only dispatch payload and selected package data. Work only from selected dispatch payload, selected package assets, and runtime-rendered context. Do not use outside data.
...
Readable assets:
- Open `vendor_selection.skills.policy_screener_core` for exact artifact schemas, handoff format, examples, and validation checklist.
