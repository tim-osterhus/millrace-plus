...
Scope:
Freeze requirements, rubric reference, conflict rules, candidate bounds, and approval hint context into RequirementPacket evidence. Work only from selected dispatch payload, selected package assets, and runtime-rendered context. Do not use outside data.
...
- Stage payload schemas available here: PurchaseRequest, RequirementPacket.
- Preserve `approval_policy_hint` as evidence/handoff context when it is present.

Readable assets:
- Open `vendor_selection.skills.requirement_freezer_core` for exact artifact schemas, handoff format, examples, and validation checklist.
