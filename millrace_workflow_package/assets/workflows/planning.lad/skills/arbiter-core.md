...
| --- | --- | --- | --- |
| `artifact_kind` | yes | string const `planning.artifacts.verdict` | Must be `planning.artifacts.verdict`. |
| `summary` | yes | string; min_length 1 | Selected-schema field. |

No additional artifact keys are allowed beyond the dispatch-selected `planning.artifacts.verdict` schema. If dispatch selects another legal Arbiter marker, use that marker's selected schema instead.
...
```json
{
  "terminal_marker": "ARBITER_COMPLETE",
  "artifact": {
    "artifact_id": "bad-lad_arbiter-wrapper",
    "artifact_kind": "planning.artifacts.verdict",
    "fields": {
      "artifact_kind": "planning.artifacts.verdict",
      "summary": "Planner summary."
...
Invalid because `artifact` is not the exact `planning.artifacts.verdict` object selected for the successful marker.
...
Lad Arbiter is complete only when it returns one legal terminal marker, one exact selected artifact body when required, and runner evidence/report text supporting the marker.
